...
function selectSource(sourceToAdd, json) {
    ...
    return api
        ...
        let sources = data.sources
            .filter((source) => existingSources.find(({ id }) => source.id === id) === undefined)
            .sort((a, b) => {
            if (a.name < b.name)
                return -1;
            if (a.name > b.name)
...
function pull(sourceFilter, json) {
    ...
    return getMasterStatus(json)
        ...
        if (result.ok) {
            ...
            report.error(`Branch ${result.branchName} was ${result.reason} ${dateFns.distanceInWords(new Date(result.closedAt), new Date())} ago. Pick another branch.`);
